...
export default function TrustBadges() {
  ...
  return (
    <>
      ...
      <section
        ref={ref}
        className="relative bg-white border-y border-(--color-border) overflow-hidden"
      ...
      >
        ...
        <div
          ...
          style={{
            backgroundImage: `
              linear-gradient(rgba(46,124,138,0.08) 1px, transparent 1px),
              linear-gradient(90deg, rgba(46,124,138,0.08) 1px, transparent 1px)
            `,
        ...
        <Container className="relative z-10">
          <div className="py-5 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 divide-x divide-(--color-border)">
            {badges.map(({ icon: Icon, title, subtitle }, i) => (
              ...
              >
                <div
                  className={`tb-icon-wrap shrink-0 w-11 h-11 rounded-xl flex items-center justify-center${visible ? " visible" : ""}`}
                  style={{
                ...
                >
                  <Icon
                    className={`tb-icon h-5 w-5${visible ? " visible" : ""}`}
                ...
                <div className="min-w-0">
                  <p
                    className={`tb-title text-sm font-bold text-(--color-secondary) truncate${visible ? " visible" : ""}`}
                    style={{ animationDelay: visible ? `${i * 600}ms` : undefined }}
                  >
                    {title}
                  </p>
                  <p
                    className={`tb-subtitle text-xs text-gray-400 leading-snug truncate${visible ? " visible" : ""}`}
                    style={{ animationDelay: visible ? `${i * 600}ms` : undefined }}
                  >
                    {subtitle}
                  </p>
