...
// Kept for API compatibility — prefer SheetContent's built-in portal.
...
>(({ side = "right", className, children, hideOverlay, overlayClassName, ...props }, ref) => {
  // Use a raw React portal instead of DialogPrimitive.Portal to avoid a
  // double-Presence bug in @radix-ui/react-dialog: DialogPortal wraps each
  // child in its own outer Presence, while DialogOverlay/Content already have
  // an inner Presence. Both watch the same animationend event and both call
  // document.body.removeChild on the same node — the second call crashes
  // because parentNode is already null. Using createPortal directly gives each
  // child exactly one Presence, which owns the sole removeChild call.
